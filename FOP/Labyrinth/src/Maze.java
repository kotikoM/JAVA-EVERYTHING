...
public class Maze extends MiniJava {
    ...
    private static class Field extends JPanel {
        ...
        public void paint(Graphics g) {
            ...
            if (maze[x][y] == WALL) {
                gradient = new GradientPaint(10, 50, Color.GRAY, getWidth(), 0, Color.DARK_GRAY);
            } else {
                gradient = new GradientPaint(0, 50, Color.WHITE, getWidth(), 0, Color.GRAY);
            }
            ...
            switch (maze[x][y]) {
                case PLAYER -> paintSymbol(g, Color.RED);
                case OLD_PATH_ACTIVE -> paintSymbol(g, Color.BLUE);
                case OLD_PATH_DONE -> paintSymbol(g, Color.GRAY);
                case PENGUIN -> drawPeng(g);
                default -> {
                }
            }
        }
        ...
        private void drawPeng(Graphics g) {
            ...
            g.drawImage(peng, 0, 0, getWidth(), getHeight(), 0, 0, peng.getWidth(null),
    ...
    private static void update(int[][] area) {
        for (int x = 0; x < maze.length; x++)
            System.arraycopy(area[x], 0, maze[x], 0, maze[0].length);
        frame.repaint();
    }
